<template>
  <v-app-bar color="grey-darken-2" :image="tlo">
    ...
    <v-toolbar-title>
      <a href="/" class="no-underline">PC Parts and service
        <v-icon size="x-large">mdi-home-circle</v-icon>
      </a>
      <v-btn v-if="userStore.ifAdmin" to="/jF8r$kL1pWz3Q@h9N7xG2kD!vA6YtO*5bTzLm0s/admin">
    ...
    <v-menu open-on-click>
      ...
      <v-list v-if="!userStore.isLoggedIn">
        ...
        <v-list-item link to="/register">
          <v-list-item-title>Załóż konto <v-icon>mdi-pencil</v-icon></v-list-item-title>
        </v-list-item>
      </v-list>
      <v-list v-else>
        <v-list-item link to="/orders">
          <v-list-item-title>Twoje Zamówienia
            <v-icon size="x-large">mdi-invoice-list</v-icon>
          </v-list-item-title>
        </v-list-item>
