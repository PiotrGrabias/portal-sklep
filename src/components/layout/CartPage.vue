<template>
      ...
      <v-col cols="12" sm="6" md="3">
        <v-card max-width="400">
          <v-card-title class="text-h5 text-center"
            >Zawartość koszyka</v-card-title
          >
        </v-card>
      </v-col>
    </v-row>
      ...
      <v-col v-for="item in cartItems" :key="item.id" cols="12" sm="6" md="3">
        <v-card class="ma-2 pa-2" hover rounded="xs" >
          ...
          <v-row class="justify-center align-center my-2">
            ...
            <v-btn v-else icon @click="removeOneItem(item.id)">
              <v-icon>mdi-delete</v-icon>
            </v-btn>
            <v-btn icon @click="addToCart(item.id)">
  ...
  <v-row no-gutters justify="center" align="center">
    <v-col align="center" cols="12" sm="6" md="3">
      <v-card elevation="50" max-width="800">
  ...
  <v-row justify="center" align="center" v-if="isLoggedIn" no-gutters>
    <v-col cols="auto" class="mx-2" > 
      <v-btn v-if="fullPrice != 0" link to="/summary" color="teal"
        >Kontynuuj</v-btn
      >
      <v-btn v-else @click="showCart = true" link to="/" color="red"
        >Brak przedmiotów w koszyku, wróć na stronę główną</v-btn
      >
    </v-col>
  </v-row>
  <ConfirmationDialog
